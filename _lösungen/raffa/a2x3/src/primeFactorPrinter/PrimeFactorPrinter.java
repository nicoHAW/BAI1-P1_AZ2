...
public class PrimeFactorPrinter {
    ...
    public void printFactorization( long number ){
        ...
        assert number > 1 : "Please enter a whole number above 1";
        
        long currentNum = number;
        String result = number + " = ";
        long currentDivider = 2; //our divider for the number, starting at 2
        
        while (currentNum > 1) {
            //run while the number we are trying to deconstruct is bigger than 1, because if it is <= 1 we cant deconstruct it any further into prime factors
            while (currentNum % currentDivider == 0) {
                //try to divide the number without remainder as often as possible for each divider
                currentNum /= currentDivider;
                //add the result to our String result
                result = result + currentDivider + (currentNum == 1 ? "" : "*");
            }
            //increase the divider each turn    
            currentDivider++;
        }
