...
/**
 * (Rectangle) Contact Analyzer for Rectangle - see task
 * 
 * @author  (your name(s)) 
...
public class ContactAnalyzer {
    ...
    /**
     * compute relation of rectangles
     * 
     * @param r1  first of two rectangles that are compared
     * @param r2  second of two rectangles that are compared
     */
    public String computeRelation( final int[][] r1,  final int[][] r2 ){
        //### Fuehren Sie Aenderungen unterhalb dieser Zeilen durch.
        //###
        //###
        //###
        ...
        // Achten Sie darauf, dass die Schreibweise des Ergebnis(-Strings) exakt(!) mit der Schreibweise in der Aufgabenstellung uebereinstimmt.
        // Damit ist NICHT der Font gemeint, sondern Klein-Großschreibung, Bindestriche, White-Spaces usw.
        // ...
        
        return null;        // <<-- Diese Zeile ist sinnvoll zu ersetzen/modifizieren. Hinter return muss das Ergebnis-String stehen.
        
        //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        // Fuehren Sie Aenderungen oberhalb dieser Zeilen durch.
        //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    }//method()
